...
class SubDomainDeletionService
{
    ...
    public function delete($server_id, $egg_id)
    {
        ...
        if (count($subdomains) > 0) {
            foreach ($subdomains as $subdomain) {
                if ($subdomain->domain_type == 1) {
                    $ownDomain = DB::table('subdomain_manager_personal_details')->where('server_id', '=', $server_id)->get();
                    if (count($ownDomain) > 0) {
                        try {
                            $key = new \Cloudflare\API\Auth\APIKey(
                                $ownDomain[0]->cloudflare_email,
                                $ownDomain[0]->cloudflare_apikey,
                            );
                            $adapter = new \Cloudflare\API\Adapter\Guzzle($key);
                            $zones = new \Cloudflare\API\Endpoints\Zones($adapter);
                            $dns = new \Cloudflare\API\Endpoints\DNS($adapter);

                            $zoneID = $zones->getZoneID($ownDomain[0]->domain);
                        } catch (\Exception $e) {
                            $error = true;
                        }

                        $protocol = $subdomain->record_type == 'SRV' ? $subdomain->protocol : '';

                        if (empty($protocol)) {
                            $subdomain_all = $subdomain->subdomain . '.' . $ownDomain[0]->domain;
                            ...
                            $subdomain_all = $protocol . '.' . $subdomain->subdomain . '.' . $ownDomain[0]->domain;
        ...
        DB::table('subdomain_manager_personal_details')->where('server_id', '=', $server_id)->delete();
    }
}
