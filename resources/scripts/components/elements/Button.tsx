...
const ButtonStyle = styled.button<Omit<Props, 'isLoading'>>`
    ${tw` rounded p-2 uppercase tracking-wide text-sm transition-all duration-150 font-bold`};

    ${(props) =>
        ((!props.isSecondary && !props.color) || props.color === 'primary') &&
        css<Props>`
            ${(props) => !props.isSecondary && tw`bg-[color:var(--ptx-button)] border-[color:var(--ptx-button)] border text-primary-50`};

            &:hover:not(:disabled) {
                ${tw`bg-[color:var(--ptx-button)] border-[color:var(--ptx-button)]`};
            }
        `};
    ...
    ${(props) =>
        ...
        css<Props>`
            ...
            &:hover:not(:disabled) {
                ...
                ${(props) => props.color === 'primary' && tw`bg-[color:var(--ptx-button)] border-[color:var(--ptx-button)] text-primary-50`};
