...
const ScreenBlock = ({ title, image, message, onBack, onRetry }: ScreenBlockProps) => (
    <PageContentBlock>
        <div css={tw`flex justify-center`}>
            <div
                css={tw`w-full sm:w-3/4 md:w-1/2 p-12 md:p-20 bg-[color:var(--ptx-secondary)] rounded-lg shadow-lg text-center relative`}
            >
                ...
                <h2 css={tw`mt-10 text-[color:var(--ptx-text)] font-bold text-4xl`}>{title}</h2>
                <p css={tw`text-sm text-[color:var(--ptx-secondarytext)]mt-2`}>{message}</p>
            </div>
        </div>
    </PageContentBlock>
);
